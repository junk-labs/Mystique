...
namespace Inscribe.ViewModels
{
    public class TabDependentTweetViewModel : ViewModel
    {
        ...
        public DelegateCommand<TextSelection> ConditionalDeselectCommand
        {
            get
            {
                if (_ConditionalDeselectCommand == null)
                    _ConditionalDeselectCommand = new DelegateCommand<TextSelection>(ConditionalDeselect);
                return _ConditionalDeselectCommand;
            }
        }

        private void ConditionalDeselect(TextSelection parameter)
        {
        ...
        #region BindTestCommand
        DelegateCommand<object> _BindTestCommand;

        public DelegateCommand<object> BindTestCommand
        {
            get
            {
                if (_BindTestCommand == null)
                    _BindTestCommand = new DelegateCommand<object>(BindTest);
                return _BindTestCommand;
            }
        }

        private void BindTest(object parameter)
        {
        }
        #endregion
        ...
        #region CopyCommand
        DelegateCommand<RichTextBox> _CopyCommand;

        public DelegateCommand<RichTextBox> CopyCommand
        {
            get
            {
                if (_CopyCommand == null)
                    _CopyCommand = new DelegateCommand<RichTextBox>(Copy);
                return _CopyCommand;
            }
        }

        private void Copy(RichTextBox parameter)
        {
            if(parameter != null)
                parameter.Copy();
        }
        ...
        DelegateCommand<RichTextBox> _SelectAllCommand;

        public DelegateCommand<RichTextBox> SelectAllCommand
        {
            get
            {
                if (_SelectAllCommand == null)
                    _SelectAllCommand = new DelegateCommand<RichTextBox>(SelectAll);
                return _SelectAllCommand;
            }
        }

        private void SelectAll(RichTextBox parameter)
        {
            if(parameter != null)
                parameter.SelectAll();
        }
        ...
        DelegateCommand<TextSelection> _SearchInKrileCommand;

        public DelegateCommand<TextSelection> SearchInKrileCommand
        {
            get
            {
                if (_SearchInKrileCommand == null)
                    _SearchInKrileCommand = new DelegateCommand<TextSelection>(SearchInKrile, CanSearchInKrile);
                return _SearchInKrileCommand;
            }
        }

        private bool CanSearchInKrile(TextSelection parameter)
        {
            return parameter != null && !String.IsNullOrEmpty(parameter.Text);
        }

        private void SearchInKrile(TextSelection parameter)
        {
            // TODO:Implementation
        }
        #endregion

        #region SearchWithGoogleCommand
        DelegateCommand<TextSelection> _SearchWithGoogleCommand;

        public DelegateCommand<TextSelection> SearchWithGoogleCommand
        {
            get
            {
                if (_SearchWithGoogleCommand == null)
                    _SearchWithGoogleCommand = new DelegateCommand<TextSelection>(SearchWithGoogle, CanSearchWithGoogle);
                return _SearchWithGoogleCommand;
            }
        }

        private bool CanSearchWithGoogle(TextSelection parameter)
        {
            return parameter != null && !String.IsNullOrEmpty(parameter.Text);
        }

        private void SearchWithGoogle(TextSelection parameter)
        {
            Browser.Start(
                    "http://www.google.co.jp/search?ie=UTF-8&q=" +
                    Dulcet.Util.HttpUtility.UrlEncodeStrict(parameter.Text, Encoding.UTF8, true));
        }
        #endregion
